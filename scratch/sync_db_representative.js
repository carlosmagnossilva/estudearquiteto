...
async function createRepresentativeData() {
  try {
    ...
    const paradas = await pool.request().query("SELECT parada_id, embarcacao_id FROM hub.fato_parada");
    ...
    for (const p of paradas.recordset) {
      ...
      const checkObra = await pool.request().query(`SELECT 1 FROM hub.fato_obra_progresso WHERE parada_id = ${id}`);
      if (checkObra.recordset.length === 0) {
          await pool.request().query(`
            INSERT INTO hub.fato_obra_progresso (parada_id, mat_realizado, mat_total, ser_realizado, ser_total, fac_realizado, fac_total)
      ...
      const checkGmud = await pool.request().query(`SELECT 1 FROM hub.fato_gmud WHERE parada_id = ${id}`);
      if (checkGmud.recordset.length === 0) {
          await pool.request().query(`
            INSERT INTO hub.fato_gmud (parada_id, total, aprovadas, adicao, exclusao, alteracao, quebra)
