...
async function inspectAndUpdate() {
  try {
    ...
    console.log("--- Inspecionando colunas de hub_frontend.fato_obra_progresso ---");
    const columns = await pool.request().query(`
      SELECT COLUMN_NAME, DATA_TYPE 
      FROM INFORMATION_SCHEMA.COLUMNS 
      WHERE TABLE_SCHEMA = 'hub_frontend' AND TABLE_NAME = 'fato_obra_progresso'
    `);
    console.table(columns.recordset);
 
    console.log("--- Inspecionando dados atuais ---");
    const data = await pool.request().query("SELECT * FROM hub_frontend.fato_obra_progresso");
    ...
    await pool.request().query(`
      ...
      SET 
        ...
        fac_realizado = 11, 
        fac_total = 20
      WHERE parada_id IN (SELECT parada_id FROM hub_frontend.fato_parada)
    `);
