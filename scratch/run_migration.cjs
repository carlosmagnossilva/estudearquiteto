// Script CJS para executar migration SQL
...
async function run() {
  ...
  try {
    ...
    console.log('[MIGRATION] Conectado ao banco:', config.database, '@', config.server);
    ...
    if (check.recordset[0].cnt > 0) {
      console.log('[MIGRATION] Tabela já existe. Exibindo estrutura atual:');
    ...
    console.log('[MIGRATION] Script executado com sucesso.');
    ...
    const confirm = await pool.request().query(`
      ...
      ORDER BY ORDINAL_POSITION
    `);
    console.log('[MIGRATION] Estrutura da tabela criada:');
